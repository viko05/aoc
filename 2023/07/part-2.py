...
class Solver:
    ...
    def compare(self, a: str, b: str):
        ...
        if '1' in set_a and not (a == '11111'):
            for card in set_a:
                if card == '1':
                    continue
                combo_map_a[a.count(card)] = card
            a_joker = a_joker.replace('1', combo_map_a[max(combo_map_a.keys())])

        if '1' in set_b and not (b == '11111'):
            for card in set_b:
                if card == '1':
                    continue
                combo_map_b[b.count(card)] = card
            b_joker = b_joker.replace('1', combo_map_b[max(combo_map_b.keys())])
